...
process_sample_for_gistic <- function(cnv_file, target_counts_file) {
  ...
  # 🔹 1. File per GISTIC: una riga per segmento
  gistic_seg_df <- cnv %>%
  ...
  # 🔹 2. File gene-level (solo se ti serve)
...
generate_gistic_uniform_file <- function(gistic_table_path, output_path = NULL) {
  ...
  if (!is.null(output_path)) {
    ...
    message("✅ File uniforme scritto in: ", output_path)
  }
  return(uniform_seg)
}
...
freq_df <- freq_df %>%
  ...
  mutate(
    ...
    # test binomiale: successi = obs_events, prove = n_samples, probabilità attesa = 0.05
...
# Ma non accetta la colonna colore direttamente → serve passare il valore e definire la palette colore
...
# Crea palette solo se l'ordine è corretto
