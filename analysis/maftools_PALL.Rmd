...
# ---- 9) Costruisci matrice regione × sample ----
mat <- df %>%
...
maf_obj <- read.maf(maf = maf_obj@data, clinicalData = params$clinical_data, cnTable = cnv_combined)
```
...
```{r all_patients_oncoplot, results='asis', dev='svg', fig.height=6, eval=FALSE}
...
for (c_path in list.dirs(params$cnvkit_out_dir, recursive = FALSE)) {
  ...
  p <- ggplot(cnr3, aes(x = pos_cum, y = log2, color = chromosome)) +
    ...
    labs(
      ...
      title = "Manhattan‑plot dei log2 ratio (bins WES)",
      color = "Cromosoma"
    )
  
  # 5. Aggiungi etichette dei cromosomi sull’asse x
...
for (c_path in list.dirs(params$raw_samples_dir, recursive = FALSE)) {
  ...
  if (grepl("^PALL[0-9A-Za-z]+-[0-9]+T[0-9A-Za-z]+$", basename(c_path))) {
    ...
    if (file.exists(file)) {
      ...
      seg_data <- read_tsv(file, col_types = cols())  # usa read.table se è .txt
...
ht <- oncoPrint(
  ...
  column_title = "CNV Oncoprint (gene × sample)",
