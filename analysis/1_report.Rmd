...
params:
  ...
  genelist_for_datatables: "/hpcnfs/scratch/P_DIMA_PALL/report_ALL/data/PALL_deletedGenes.csv"
...
oncoplot_margin <- 6
oncoplot_font_size <- 1.3

genelist_for_datatables <- read_csv(params$genelist_for_datatables, show_col_types = FALSE) %>%
  pull(Gene)
...
The MAF object is preprocessed to:
  ...
  - filter the variant list by:
    ...
    - discarding not important variants ("Silent", "Intron", "5'UTR")
...
### Most pathogenic in VCC, all samples

#### Tier I and II genes {.tabset}
...
main_onco <- oncoplot(
  ...
  barcode_mar = oncoplot_margin, 
  clinicalFeatures = c("T1221", "Sample_group"), 
  SampleNamefontSize = oncoplot_font_size,
...
#### Tier I, II and III genes {.tabset}
...
### Most pathogenic in CC, all samples
...
### Most pathogenic in VCC, only diagnosis and relapse 1 samples
...
### Most pathogenic in CC, only diagnosis and relapse 1 samples
...
## CN Oncoplots, DRAGEN annotation/seg files
...
```{r oncoplot_dragen_cnbands_filter1}
plot_cn_oncoprint(
...
```{r oncoplot_dragen_cnbands_filter2}
plot_cn_oncoprint(
...
```{r oncoplot_dragen_cnbands_filter3}
plot_cn_oncoprint(
...
### All samples, grouped by patient {.tabset}

#### CN Oncoplot, all samples, tier I

```{r oncoplot_dragen_cnbands_filter12}
plot_cn_oncoprint(
  cn_block = cn_oncoplot_data$tier_I,
  title = paste(
    "Top",
  ...
  sample_order = sample_order_patient_adj,
  remove_empty_columns = FALSE
)
```
...
```{r oncoplot_dragen_cnbands_filter22}
plot_cn_oncoprint(
  cn_block = cn_oncoplot_data$tier_I_II,
  title = paste(
    "Top",
...
```{r oncoplot_dragen_cnbands_filter32}
plot_cn_oncoprint(
  cn_block = cn_oncoplot_data$tier_I_II_III,
  title = paste(
    "Top",
...
### Only genes from genelist

```{r plot_table4, echo=FALSE}
DT::datatable(
  gene_cn_final_table %>%
    filter(gene %in% genelist_for_datatables),
  extensions = 'Buttons',
  options = list(
    dom = 'Blfrtip',
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
